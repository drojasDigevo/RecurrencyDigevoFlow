...
module.exports = async function (context, req) {
    try {
        ...
        if (!startDate || !planName || !frequency || !quantity || !totalQuantity || !idSubscription) {
            ...
            context.res.body = { error: "Alguno de los siguientes valores no existen: [idSubscription, startDate, planName, frequency, quantity, totalQuantity]" }
            return
        }
        ...
        if (subExists) {
            context.res.status = 400
            context.res.body = { error: `Ya existe una subscripción con idSubscription: ${idSubscription}` }
            return
        }
        ...
        if (!plan) {
            context.res.status = 400
            context.res.body = { error: `El plan con el nombre "${planName}" no existe` }
            return
        }
        ...
        if (!(
            ...
            context.res.status = 400
            context.res.body = { error: `"totalQuantity" has to be ${plan.totalQuantityDispatched} and "frequency" has to be one of the values: [${plan.deliveryFrequencyOptions.join(', ')}]` }
            return
        }
